...
#We have 827 records and it takes about 104-105 days for the flowers to blossom
...
#df['month'] = pd.to_datetime(df.full_flowering_date, format='%m%d', errors='coerce').dt.month_name()
...
df.month.value_counts().plot(kind="barh", figsize=(15,5), title="How many blossomings happened each month")
...
df['date'] = pd.to_datetime(df.full_flowering_date, format='%m%d', errors='coerce').dt.strftime('%B %d')
df.sample(5)
...
df['ad'] = pd.to_datetime(df.ad, format='%Y')
df.ad
#Just kidding -- just found out Soma said to skip this because pandas can't handle years this far back
